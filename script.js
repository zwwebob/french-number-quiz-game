...
nextBtn.onclick = () => {
    if (questionCount < selectedQuestions.length - 1) {
        ...
        nextBtn.classList.remove('active'); //when the next button is clicked, it will be unclickable in the next question.
    }
    else {
...
//get questions and options from arrays
...
function optionSelected(answer) {
    ...
    //if the user has selected, disable other options
    for (let i = 0; i < allOptions; i++) {
...
function showResultBox() {
    ...
    scoreText.textContent = `Your Score is ${userScore} out of ${selectedQuestions.length}`;

    const circularProgress = document.querySelector('.circular-progress');
    const progressValue = document.querySelector('.progress-value');
    let progressStartValue = 0;
    let progressEndValue = Math.floor((userScore / selectedQuestions.length) * 100);
    let speed = 10;
